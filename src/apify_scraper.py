...
def _resolve_data_dir() -> Path:
    """Read data_dir from config.yaml (default: 'data'). Creates the dir."""
    try:
...
def _latest_post_info(item: dict) -> tuple[Optional[str], Optional[str]]:
    """Extract (latest_post_timestamp_iso, earliest_post_timestamp_iso) from latestPosts.

    apify/instagram-profile-scraper returns latestPosts: list of {timestamp, ...}.
    ...
    ts = []
    for p in posts:
        t = p.get("timestamp") or p.get("taken_at") or p.get("takenAt")
        if t:
            ts.append(str(t))
    if not ts:
        return None, None
...
def _normalize_profile(item: dict) -> dict:
    ...
    latest_post, earliest_post = _latest_post_info(item)
    return {
        ...
        "is_business_account": item.get("isBusinessAccount", item.get("is_business_account", False)),
        ...
        "latest_post_timestamp": latest_post,
        "earliest_post_timestamp": earliest_post,
        "joined_recently": item.get("joinedRecently") or item.get("joined_recently"),
    }
